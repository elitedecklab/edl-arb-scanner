#!/usr/bin/env python3
"""
EDL Arbitrage Scanner - v9 (cloud-ready: runs on Render OR locally)
- Locally: double-runnable like before (keys from D:\Arbitrage Scanner\*.txt, opens browser).
- On Render: set env vars EBAY_APP_ID, EBAY_CERT_ID, POKEDATA_KEY, EDL_DIR=/tmp/edl.
  Render serves the dashboard publicly; view it anywhere, even with the laptop off.

v9 changes vs v8:
  - keys from env vars first (falls back to local *.txt files).
  - dashboard server binds 0.0.0.0:$PORT in the cloud (127.0.0.1:8787 locally).
  - data/cache dir overridable via EDL_DIR; no local-file dependency in the cloud.
  - web server starts BEFORE the first catalog build so Render sees the open port fast.
  - browser auto-open skipped in the cloud.
"""
...
DASH_PORT     = int(os.environ.get("PORT", "8787"))       # Render injects PORT
...
JUNK = ["accessories", "accessory", "sleeve", "empty", "opened", "open box", "code", "lot",
        ...
        "japanese", "japan", "korean", "chinese", "french", "german", "spanish", "italian",
...
def eval_bin(item, prod):
    ...
    if known:
        ...
        if total > market * BIN_MAX_RATIO: return None
        ratio = total / market
    else:
        if price > market * SHIP_UNKNOWN_MAX_RATIO: return None
        total, ratio = price, price / market
    if ratio < BIN_MIN_RATIO: return None
...
def eval_auction(item, prod, token):
    ...
    ratio = cost / market
    if cost > market * AUCTION_MAX_RATIO or ratio < AUCTION_MIN_RATIO: return None
    o = base_opp(item, prod)
...
def scan(token, universe, existing_ids, cycle_iso):
    ...
    for prod in universe:
        market = prod["market"]
        ceil = round(market * BIN_MAX_RATIO, 2)
        ...
        for it in ebay_search(token, prod["query"], bin_filt, sort="price", limit=BIN_LIMIT):
            ...
            if not iid or iid in seen or not title_ok(title, prod["require"], prod["exclude"]): continue
            if is_true_case(title) != prod["is_case"]: continue
            o = eval_bin(it, prod)
            if o:
...
def validate(token, opps, uni_by_pid, cycle_iso):
    ...
    for o in opps:
        ...
        title = o.get("title", "")
        if not title_ok(title, prod["require"], prod["exclude"]): continue
        if is_true_case(title) != prod["is_case"]: continue
        d = ebay_get_item(token, o["item_id"])
        if not d: continue
        ...
        if o["kind"] == "BIN":
            ...
            ok = (total <= market * BIN_MAX_RATIO) if known else (price <= market * SHIP_UNKNOWN_MAX_RATIO)
            if not ok or ratio < BIN_MIN_RATIO: continue
            o.update({"item_price": round(price, 2), "shipping": (round(ship, 2) if known and ship is not None else None),
                      "shipping_known": known, "total": round(total, 2), "ratio": round(ratio, 3),
                      "market": market, "last_validated": cycle_iso})
        else:
            ...
            if cost > market * AUCTION_MAX_RATIO or ratio < AUCTION_MIN_RATIO or \
            ...
            o.update({"current_bid": round(bid, 2), "min_next_bid": money(d.get("minimumPriceToBid")),
                      ...
                      "shipping": (round(ship, 2) if known and ship is not None else None),
                      "shipping_known": known, "ratio": round(ratio, 3), "market": market, "last_validated": cycle_iso})
...
def main():
    ...
    render_dashboard(opps, now_utc().isoformat())           # write something before the port opens
    threading.Thread(target=serve_dashboard, daemon=True).start()
    time.sleep(0.5)
    log(f"dashboard serving on {DASH_HOST}:{DASH_PORT}" + ("" if IS_CLOUD else "  (point Cloudflare Tunnel here, or open http://localhost:%d/)" % DASH_PORT))
    open_dashboard()
    setid_map = build_setid_map()
    universe = build_universe(keys, setid_map, dump=True)
    write_universe_report(universe)
    by_t = Counter(p["type_label"] for p in universe)
    log(f"universe: {len(universe)} products across {len(PRIORITY_SETS)} sets")
    log("  " + ", ".join(f"{k}:{v}" for k, v in sorted(by_t.items())))
    while True:
        cstart = now_utc(); cycle_iso = cstart.isoformat()
        try:
            tok = keys.ebay_token()
            universe = build_universe(keys, setid_map)
